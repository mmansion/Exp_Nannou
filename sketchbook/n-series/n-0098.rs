...
fn view(app: &App, m: &Model, frame: Frame) {
    ...
    for i in 0..count {
        ...
        let x = rrand;
        draw.rotate(PI).ellipse().x_y(x, y).w_h(point_size, point_size).color(BLACK);
        // for ii in 0..3 {
    ...
    for i in 0..count/2 {
        ...
        let y = map_range(i as f32, 0.0, (count/2) as f32, -rect_width/2.0, rect_width/2.0);
        draw.ellipse().x_y(x, y).w_h(point_size/2.0, point_size/2.0).color(BLACK);
        // for ii in 0..3 {
        //     let y_off = 50.0 * ii as f32;
        // }
    }
