...
fn update(app: &App, model: &mut Model, _update: Update) {
    ...
    for inc in model.ibin_1.iter_mut() {
        *inc += 0.008;
    }
    for inc in model.ibin_2.iter_mut() {
        *inc += 0.002;
    }
...
fn view(app: &App, model: &Model, frame: Frame) {
    ...
    let col1 = hsv(time * 0.06, 1.0, 1.0);
    let col2 = hsv(time * 0.05, 1.0, 1.0);
    let col3 = hsv(time * 0.04, 1.0, 1.0);
    let col4 = hsv(time * 0.03, 1.0, 1.0);
    let col5 = hsv(time * 0.02, 1.0, 1.0);
    let col6 = hsv(time * 0.01, 1.0, 1.0);
    ...
    let mut points_5 = Vec::new();
    let mut points_6 = Vec::new();
    ...
    let draw = draw.x_y(model.mover.y, model.mover.x);
    let draw = draw.rotate(180.0);
