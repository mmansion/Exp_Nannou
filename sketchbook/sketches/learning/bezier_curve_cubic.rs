...
fn view(app: &App, m: &Model, frame: Frame) {
    ...
    // draw starting point
    draw.ellipse().xy(m.p1).radius(POINT_SIZE).color(RED);

    // draw control point 1
    draw.ellipse().xy(m.cp1).radius(POINT_SIZE).color(BLUE);

    // draw control point 2
    draw.ellipse().xy(m.cp2).radius(POINT_SIZE).color(BLUE);

    // draw end point
    draw.ellipse().xy(m.p2).radius(POINT_SIZE).color(RED);
...
fn mouse_pressed(app: &App, m: &mut Model, btn: MouseButton) {
    ...
    if dist_p1 < POINT_SIZE {
        m.point_active = 1;
    } else if dist_cp1 < POINT_SIZE {
        m.point_active = 2;
    } else if dist_cp2 < POINT_SIZE {
        m.point_active = 3;
    } else if dist_p2 < POINT_SIZE {
        m.point_active = 4;
    }
}
