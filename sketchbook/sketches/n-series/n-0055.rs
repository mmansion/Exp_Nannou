...
impl Anim {
    ...
    pub fn set_color(&mut self, color:Hsva){
        self.color = color;
    }
...
fn model(app: &App) -> Model {
    ...
    let mut color = hsva(1.0, 1.0, 1.0, 1.0);

    for i in 0..grid.points.len() {
        touched_points.push(true);
        anims.push( Anim::new(grid.points[i], 0.1, POINT_SIZE, POINT_SIZE * 6.0));
    }
        ...
        for v in 0..m.vehicles.len() {
            for i in 0..m.grid.points.len() {
                if m.vehicles[v].hasCollision(m.grid.points[i], POINT_SIZE) {
                    if(!m.anims[i].animate) {
                        m.touched_points[i] = true; 
                        m.anims[i].animate = true;
                        m.anims[i].set_color(m.color);
                    }
                }
            }
        }
