...
fn update(app: &App, m: &mut Model, _update: Update) {
    ...
    for i in 0..m.particles.len() { 
        ...
        if !m.vbow.left_line.point_above_line(m.particles[i].pos, 0.0, -m.particles[i].size/2.0) && 
        ...
        if !m.vbow.right_line.point_above_line(m.particles[i].pos, 0.0, -m.particles[i].size/2.0) && 
