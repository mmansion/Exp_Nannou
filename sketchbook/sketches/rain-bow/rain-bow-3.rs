...
fn update(app: &App, m: &mut Model, _update: Update) {
    ...
    for i in 0..m.particles.len() {
        ...
        if !m.vbow.left_line.point_above_line(
            m.particles[i].position,
            0.0,
            -m.particles[i].display_size / 2.0,
        ...
        if !m.vbow.right_line.point_above_line(
            m.particles[i].position,
            0.0,
            -m.particles[i].display_size / 2.0,
...
fn mouse_pressed(app: &App, m: &mut Model, b: MouseButton) {
    ...
    m.particles
        .push(Particle::new(app.mouse.x, HEIGHT / 2.0, PARTICLE_SIZE));
}
