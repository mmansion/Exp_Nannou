...
fn model(app: &App) -> Model {
    ...
    Model { points, vehicles, this_capture_frame, last_capture_frame, debug, d }
} 
...
fn view(app: &App, model: &Model, frame: Frame) {
    ...
    for i in 0..model.points.len() {
        ...
        let color = hsva ( map_range( abs(app.time.sin() * 0.001), 0.4, 0.9, 0.3, 0.75), 1.0, 1.0, 0.1);
...
fn display(vehicle: &Vehicle, draw: &Draw, app: &App, num:i32) {
    ...
    if history.len() > 1 {
        ...
        draw.polyline().caps_round().weight(num as f32).points_colored(vertices);
    }
